package com.example.anees.enums

import com.example.anees.R

enum class RecitersEnum(val reciter : String, val url : String, val image : Int , val description : String) {

    Abdelbaset("عبدالباسط عبدالصمد","https://server7.mp3quran.net/basit/Almusshaf-Al-Mojawwad/",R.drawable.abdulbasit,"المصحف المجود - المصحف المجود"),
    Maher("ماهر المعيقلي","https://server12.mp3quran.net/maher/",R.drawable.maher,"حفص عن عاصم - مرتل"),
    Hosary("محمود خليل الحصري","https://server13.mp3quran.net/husr/Rewayat-Warsh-A-n-Nafi",R.drawable.hosary,"ورش عن نافع - مرتل"),
    Banna("محمود علي البنا","https://server8.mp3quran.net/bna/Almusshaf-Al-Mojawwad/",R.drawable.banna,"المصحف المجود - المصحف المجود"),
    Refaat("محمد رفعت","https://server14.mp3quran.net/refat/",R.drawable.refaat,"حفص عن عاصم - مرتل"),
    Menshawy("محمد صديق المنشاوي","https://server10.mp3quran.net/minsh/Almusshaf-Al-Mojawwad/",R.drawable.menshawy,"المصحف المجود - المصحف المجود"),
    Yasin("القارئ ياسين","https://server11.mp3quran.net/qari/",R.drawable.yasin,"ورش عن نافع - مرتل"),
    allhaidan("محمد اللحيدان","https://server8.mp3quran.net/lhdan/",R.drawable.allhaidan,"حفص عن عاصم - مرتل"),
    Idres("إدريس أبكر","https://server6.mp3quran.net/abkr/",R.drawable.idres,"حفص عن عاصم - مرتل"),
    Galel("خالد الجليل","https://server10.mp3quran.net/jleel/" ,R.drawable.galel,"حفص عن عاصم - مرتل"),
    Wadiee("وديع اليمني","https://server6.mp3quran.net/wdee3/",R.drawable.wadiee,"حفص عن عاصم - مرتل"),
    Kurdi("ناصر القطامي","https://server6.mp3quran.net/qtm/",R.drawable.qtamy,"حفص عن عاصم - مرتل"),
    Hazza("هزاع البلوشي","https://server11.mp3quran.net/hazza/",R.drawable.hazza,"حفص عن عاصم - مرتل"),
    Badr("بدر التركي","https://server10.mp3quran.net/bader/Rewayat-Hafs-A-n-Assem/",R.drawable.badr,"حفص عن عاصم - مرتل"),

}